// src/app/pages/crm/outlook-callback/outlook-callback.component.ts
import { Component, OnInit, inject } from '@angular/core';
import { ActivatedRoute } from '@angular/router';
import { CommonModule } from '@angular/common';

@Component({
  selector: 'wt-outlook-callback',
  standalone: true,
...
<div style="position:fixed;inset:0;background:#f9fafb;display:flex;align-items:center;justify-content:center;padding:24px">
  <div style="background:white;border-radius:16px;padding:40px 48px;max-width:480px;width:100%;text-align:center;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    ...
    <ng-container *ngIf="status === 'connected'">
      ...
      <div style="font-size:20px;font-weight:800;color:#111827;margin-bottom:8px">Outlook połączony!</div>
      <div style="font-size:14px;color:#6b7280;margin-bottom:28px;line-height:1.6">
        Twoje konto Outlook zostało pomyślnie połączone z aplikacją.<br>
        Możesz zamknąć to okno i wrócić do pracy w CRM.
      </div>
      <button (click)="close()"
              style="background:#0078d4;color:white;border:none;border-radius:8px;padding:10px 28px;font-size:14px;font-weight:600;cursor:pointer">
        Zamknij okno
      </button>
    </ng-container>
    ...
    <ng-container *ngIf="status === 'error'">
      ...
      <div style="font-size:14px;color:#6b7280;margin-bottom:8px;line-height:1.6">
        Nie udało się połączyć konta Outlook.
      </div>
...
export class OutlookCallbackComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.status === 'connected') {
      // localStorage storage-event — same cross-tab communication pattern as Gmail callback
      localStorage.setItem('outlook_oauth_connected', String(Date.now()));
    }

    // BroadcastChannel — primary mechanism
    try {
      const bc = new BroadcastChannel('outlook-oauth');
      bc.postMessage({ type: 'outlook-oauth-result', status: this.status });
      bc.close();
    } catch (_) {}
    ...
    if (window.opener) {
      try {
        window.opener.postMessage({ type: 'outlook-oauth-result', status: this.status }, window.location.origin);
      } catch (_) {}
    }
  }
