...
export class CrmPartnerDetailComponent implements OnInit, OnDestroy {
        ...
        if (!wasReply && this.partner) {
          ...
          this.getPartnerThreadObs(replyThreadId).subscribe({
            next: res => this.zone.run(() => {
              ...
              // partner.activities refreshes (poller, tab switch, "Sprawdź nowe")
  ...
  private getPartnerThreadObs(threadId: string) {
    ...
    if (act?.email_provider === 'outlook') {
      return this.api.getPartnerEmailThreadOutlook(this.pid, threadId);
    }
    if (act?.email_provider === 'zoho') {
      return this.api.getPartnerEmailThreadZoho(this.pid, threadId);
    }
